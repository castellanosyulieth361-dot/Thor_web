...
const crearReporteSchema = z.object({
  ...
  situacion:   z.enum([
    "incidente", "impacto_ambiental", "error_info_tecnica",
    "incumplimiento_parametros", "acto_seguro", "acto_inseguro", "condicion_segura",
  ]),
...
router.post("/", requireAuth, async (req, res) => {
  try {
    ...
    if (data.area === "otro" && !data.area_otro?.trim()) {
      return res.status(400).json({ message: "Debes especificar el área cuando seleccionas 'Otro'." });
    }
...
router.get("/", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    if (desde)      { params.push(desde);      where.push(`ro.fecha >= $${params.length}::date`); }
    if (hasta)      { params.push(hasta);      where.push(`ro.fecha <= $${params.length}::date`); }
    if (area)       { params.push(area);       where.push(`ro.area = $${params.length}`); }
    if (situacion)  { params.push(situacion);  where.push(`ro.situacion = $${params.length}`); }
    if (usuario_id) { params.push(usuario_id); where.push(`ro.reportado_por = $${params.length}`); }
...
router.get("/historial/:anio/:mes", requireAuth, requireAdmin, async (req, res) => {
  try {
    const anio = Number(req.params.anio);
    const mes  = Number(req.params.mes);
    if (isNaN(anio) || isNaN(mes)) return res.status(400).json({ message: "Año o mes inválidos." });

    const { area, situacion, usuario_id } = req.query;
    const params = [anio, mes];
    const extra  = [];

    if (area)       { params.push(area);       extra.push(`ro.area = $${params.length}`); }
    if (situacion)  { params.push(situacion);  extra.push(`ro.situacion = $${params.length}`); }
    if (usuario_id) { params.push(usuario_id); extra.push(`ro.reportado_por = $${params.length}`); }
...
router.get("/:id", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    if (rRes.rowCount === 0) return res.status(404).json({ message: "Reporte no encontrado." });
...
router.post("/:id/gestion", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const reporteRes = await pool.query(
      `SELECT estado FROM reportes_observacion WHERE id = $1`, [id]
    );
    if (reporteRes.rowCount === 0) return res.status(404).json({ message: "Reporte no encontrado." });
    ...
    const gRes = await pool.query(
      `INSERT INTO reportes_observacion_gestion
       ...
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [id, req.user.id, data.tipo_accion, data.descripcion, data.cierra_reporte]
    );
